...
## ✅ STATUS (update terakhir — pemulihan repo + Fase 33 dimulai)
...
- **Fase 33 (RAB/BoQ ↔ jadwal → opname & termin subkon)**: sedang dikerjakan — lihat §FASE 33.
...
## 🚧 FASE 33 — RAB/BoQ ↔ ITEM JADWAL → OPNAME & TERMIN SUBKON (SEDANG DIKERJAKAN)
...
Fase 31 & 32 **selesai & terverifikasi**; Fase 33 sedang dikerjakan (lihat §FASE 33). Kandidat setelahnya:
1. **Jadwal massal per blok/cluster** — buat jadwal banyak unit sekaligus + geser tanggal serentak saat proyek mundur (masih ada unit belum terjadwal). ← disetujui owner sebagai Fase 34
2. **Papan mandor offline-tolerant** — antrean unggahan bila sinyal lapangan hilang. ← Fase 35
3. **Terapkan rekomendasi kalibrasi sekali klik** — dari Analitik Telat langsung mengubah durasi/waktu tunggu pada template (sekarang masih manual di editor).
